# Project118
----------------------------------------
http://iatse118.com rebuild

A good use case for Laravel.


Project Title: Local 118 Rebuild

Background

IATSE Local 118 in Vancouver has a website which has served its needs well in the past. With more recent influences such as innovation in how to present content on the web, greater adoption of the site and growth in the local, a new instance of the website that makes use of current software will better serve the local. It will allow for management of more timely information, addition of online services and the reorganization of current ones to best suit the needs of both member users and public visitors of the website. 

Peter Gordon is a member in good standing with IATSE 118, with approximately two years experience with the Local. Peter has over 18 years experience in Web application development, and has approached other members about the possibility of rebuilding the website. Peter offers to redevelop the current site on behalf of the Local as a pro bono project as he has interest in doing it as a portfolio piece. 

High-Level Project Proposal

Rebuild the site in current software in a development environment, making it possible to demonstrate the site offline. The site news content will be copied in and reorganized. User profiles can be generated from a fake data generator. 

Peter will inform the key stakeholders and seek further approval to build out the project with actual member data and will meet to discuss changes to website features and the addition of new ones. A Waterfall/Agile project management methodology must be adopted to begin and complete the second phase of activity. 

The second phase of the project will import member data and content, build out improvements for the site and plan the user migration to the new site, change to the new site and retire the current website. 

Possible New Features

Dues tracking. Users to be made aware of status of membership dues.
Dynamic generation of member list.
Moderated discussion. 
Administrative privileges for different areas of the site.
Other features as needed.


Development Schedule 

Peter Gordon will perform the development work for this project on a casual basis, time permitting. Delivery and presentation of the first phase will be in summer 2019. 


Resources 

Technical Stack
The website will be rebuilt with:
Laravel Framework.
Twitter Bootstrap Library for layout. 
Vue.js for dynamic generation of resources. 
MySQL/MariaDB for database.
BitBucket account for website version control with GIT.
LaraDock for development environment.
Docker containerization for deployment.
Linux operating system.
PHPStorm & emacs for editors.
Google Drive, Trello, Asana, and BitBucket issues tracker will be used for project. communication and documentation. 
Google Analytics. Possibly New Relic or DataDog.
Email: MailGun as a proposed initial solution, also Gmail for Business. 
Hosting: Vultur, DigitalOcean or Amazon Web Services. 

Budget

Peter has spoken with President Corin Gutteridge in February 2019 about the website redesign. While Pres. Gutteridge has indicated that there may be some office funds available to support the work in the project, this is not a concern in the primary stages of the project. Billable work may be negotiated in a later phase of project activity. 

The software and services initially as proposed are all Free Open Source Software. Some of the other services do offer premium features and they will be considered on an as-needed basis. 
The new website hosting cost would be between $5 - $20 per month. 

Key Stakeholders

President Corin Gutteridge.
Brother Andrew McLean Youth Committee.
Brother Don Underhill (previous developer).
IATSE 118 Executive Board.
Peter Gordon, IATSE 118 Member, Web Application Developer.
Remick Tadesse-Lee, Technical Advisor and Mentor to Peter.

Please note: a primary client representative still needs to be determined. One person from the Local will need to act as the ‘customer’ or ‘decision maker’, at the time of formal acceptance of the project for further development, or second phase. 

For further questions or comments, Peter Gordon can be reached at 778.836.8981, or at superwebdeveloper@gmail.com. 


