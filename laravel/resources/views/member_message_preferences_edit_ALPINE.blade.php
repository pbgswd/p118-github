                ...
                @foreach($data['topic_subscription_options'] as $topic)
                    <div class="input-group mb-3">
                        <div class="input-group-text">
                            <input
                                x-bind:checked="{{ array_key_exists($topic['slug'], $data['selections']['topics']) ? 'deselectalltopic'  : 'selectalltopic'}}"

                                   class="form-check-input mt-0" id="checkbox-topic-{{$topic['slug']}}" name="message_selections[topic][]" type="checkbox"
                       ...
                       <div class="col">{{array_key_exists($topic['slug'], $data['selections']['topics']) ? ' checked ' : ''}}</div>
                    </div>
                @endforeach
                <div class="input-group mb-3">
