@extends('layouts.dashboard',  ['title_icon' => '<i class="fas fa-list"></i>', 'title' => 'List Committees'])
@section('content')
    <div class="container">
        <h3>
            <span class="badge badge-primary badge-pill">
               {{$data['committees']->count()}}
            </span>
            Committees.
            @can('create committee')
                |
                <a href="{{ route('committee_create') }}">
                    Create new Committee
                    <i class="far fa-arrow-alt-circle-right"></i>
                </a>
            @endcan
        </h3>
    </div>
    @can('manage committee')
        <div class="row mt-lg-5">
            <div class="col-md-12">
                <h3>Your committees</h3>
            </div>
        </div>
        @if($data['manage committees'] !== null)
            <div class="table-responsive">
                <table class="table table-striped table-sm">
                    <thead>
                    <tr>
                        <th> Name </th>
                        <th>  </th>
                    </thead>
                    <tbody>
                        @forelse ($data['manage committees'] as $m )
                            <tr>
                                <td>
                                    <h4>
                                        <a href="{{route('admin_committee_show', $m->slug)}}"
                                           title="Edit {{ $m->name }}">
                                            {{ $m->name }}
                                        </a>
                                    </h4>
                                </td>
                                <td>
                                    <a href="{{route('admin_committee_show', $m->slug)}}"
                                       title="Edit {{ $m->name }}">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="2">No committees assigned</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        @endif
    @endcan
    @role('super-admin')
        <div class="row mt-lg-5">
            <div class="col-md-12">
                <h3>All committees</h3>
            </div>
        </div>
        <form name="delete" method="POST" action="{{route('committee_destroy')}}">
            {!! csrf_field() !!}
            {!! method_field('DELETE') !!}
            <div class="form-group">
                <div class="table-responsive">
                    <table class="table table-striped table-sm">
                        <thead>
                            <tr>
                                <th> @sortablelink('id','#') </th>
                                <th> @sortablelink('title', 'Title') </th>
                                <th> @sortablelink('live', 'Is Live?') </th>
                                <th> Created By </th>
                                <th>
                                    @can('manage committee')
                                        Edit
                                    @endcan
                                </th>
                                <th> @sortablelink('created_at', 'Created At') </th>
                                <th> @sortablelink('updated_at', 'Updated At') </th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ( $data['committees'] as $c )
                                <tr>
                                    <td>
                                        @can('delete committee')
                                            <div class="checkbox">
                                                <label>
                                                    <input type="checkbox" name="id[]" value="{{$c->id}}" />
                                                </label>
                                            </div>
                                        @endcan
                                    </td>
                                    <td>
                                        <h4>
                                            <a title="{{ $c->name }}"
                                               href="{{ route('admin_committee_show', $c->slug) }}">
                                                {{ $c->name }}
                                            </a>
                                        </h4>
                                    </td>
                                    <td> {!! $c->live ? "<i class='fas fa-check'></i>" :
                                            "<i class='far fa-times-circle'></i>" !!}
                                    </td>
                                    <td>{{ $c->creator->name }}</td>
                                    <td>
                                        @can('manage committee')
                                            <a href="{{ route('committee_edit', $c->slug) }}"
                                               title="Edit {{ $c->name }} ">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                        @endcan
                                    </td>
                                    <td> {{ $c->created_at->format('F j Y H:i:s') }} </td>
                                    <td> {{ $c->updated_at->format('F j Y H:i:s') }} </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="7">
                                        No committees yet.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            </div>
            <div class="row mb-lg-5">
                <div class="col">
                    @can('delete committee')
                        @if($data['committees']->count() > 0)
                            <i class="far fa-trash-alt fa-2x"></i>
                            <input class="btn btn-outline-danger" type="submit" value="Delete Selected">
                        @endif
                    @endcan
                </div>
                <div class="col-6">
                    <div class="list-group">
                        <ul class="pagination">
                             {{$data['committees']->links()}}
                        </ul>
                    </div>
                </div>
                <div class="col"></div>
            </div>
        </form>
    @endrole
@endsection
