        ...
        <div class="row p-3">
            @foreach ( $data['organizations'] as $organization )
                <div class="col-12 col-md-4 mt-3 p-2">
                    <div class="border border-dark rounded h-100 w-100 p-2 text-center d-flex align-items-center justify-content-center">
                        <a href="{{ route('organization', $organization->slug) }}" class="">
                            @if($organization->image)
                                <img src="{{asset('storage/public/'. $data['tn_prefix'].$organization->image)}}"
                                     class="img-fluid rounded mb-3" title="{{$organization->name}}"/> <br />
                            @endif
